...
export default function InputMoney({ value, initialAmount = true ,onChangeValue, prefixFunc, prefixState }) {
   
    //const [prefix, setPrefix] = useState(value <= 0 ? false : true)
    const [prefix, setPrefix] = useState(initialAmount ? true : prefixState)


    function changePrefix(){
        setPrefix(!prefix)
        onChangeValue(value * -1)
        prefixFunc && prefixFunc(!prefix)
